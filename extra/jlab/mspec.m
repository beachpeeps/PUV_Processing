...
%cellout{length(cellout)+1} = f;
cellout{length(cellout)+1} = mmatx;
...
cv=cv-vmean(cv,1);
[f,spp,snn]=mspec(cv,ones(N,1)./sqrt(N));
reporttest('MSPEC periodogram matches variance exactly for zero-mean complex signal and odd length', aresame(sum(spp+snn)./N,sum(abs(cv).^2)./N,1e-10))

N=1001;
cv=randn(N,1)+sqrt(-1)*randn(N,1)+17;
[f,spp,snn]=mspec(cv,ones(N,1)./sqrt(N));
reporttest('MSPEC periodogram matches variance exactly for non-zero-mean complex signal and odd length', aresame(sum(spp+snn)./N,sum(abs(cv).^2)./N,1e-10))
...
cv=cv-vmean(cv,1);
[f,spp,snn]=mspec(cv,ones(N,1)./sqrt(N));
reporttest('MSPEC periodogram matches variance exactly for zero-mean complex signal and even length', aresame(sum(spp+snn)./N,sum(abs(cv).^2)./N,1e-10))

N=1000;
cv=randn(N,1)+sqrt(-1)*randn(N,1)+17;
[f,spp,snn]=mspec(cv,ones(N,1)./sqrt(N));
reporttest('MSPEC periodogram matches variance exactly for non-zero-mean complex signal and even length', aresame(sum(spp+snn)./N,sum(abs(cv).^2)./N,1e-10))
