...
[fm, Suu, ~, ~, ~, ~] = get_spectrum(U, 1200, 2, 0.05);
[~, Svv, ~, ~, ~, ~] = get_spectrum(V, 1200, 2, 0.05);
[~, Spp, ~, ~, ~, ~] = get_spectrum(P, 1200, 2, 0.05);
...
%Spec = struct('fm', fm, 'Spp', Spp,'Suu', Suu, 'Svv', Svv, 'i_ig', i_ig, 'i_swell', i_swell);
%% DONE Spectra 
% % DEPTH CORRECTION -------------------------------------------------------------
...
correction = zeros(size(fm(ii)))';
convert = zeros(size(fm(ii)))';
...
Sppt = Spp(ii);
SSE = Sppt .* (correction.^2) ;   % correct pressure for attentuation to get sea-surface elevation
SSEt = SSE(ii);
...
UUpres = Suut .* (convert.^2) ;   % convert velocities into "equivalent pressure" (to compare with PP)
VVpres = Svvt .* (convert.^2) ;
UVpres = Suvt .* (convert.^2) ;
%units = m^2/s^2 /Hz* s^2 = m^2*s

PUpres = Sput .* convert ;   % convert x-spectra as well (but conversion is not squared here)
PVpres = Spvt .* convert ;
...
cohPUpres = sqrt( (coPUpres.^2 + quPUpres.^2) ./ (Sppt.* UUpres) );
phPUpres  = (180/pi) .* atan2( quPUpres , coPUpres );
cohPVpres = sqrt((coPVpres.^2 + quPVpres.^2)./ (Sppt.* VVpres));
...
% % Elgar 1999 - Directional spreading of waves in the nearshore
% 
a1 = coPUpres ./ sqrt( Sppt .* ( UUpres + VVpres ) );
b1 = coPVpres ./ sqrt( Sppt .* ( UUpres + VVpres ) );
...
%Eflux_dir = struct('posX', posX(ii), 'posY', posY(ii), 'negX', negX(ii), 'negY', negY(ii), 'Cg', Cg(ii), 
Eflux = struct('depth', depth);
% 
...
ids = struct('i_swell', i_swell, 'i_ig', i_ig);%, 'k', k, 'omega', omega, 'fcentroid_swell', fcentroid_swell, 'fpeak',fpeak,'Tm',Tm, 'fspread',fspread);
%% ----------------------------------------------------------------------------
A = structurize;
    function A = structurize
        A = struct('Spec',Spec, 'FC', FC, 'Eflux', Eflux, 'ids', ids);%, 'ztest', ztest,'Hsig', Hsig,'FC', FC, 'coh', coh, 'dir', dir,'RS', RS,'Eflux', Eflux_dir, 'ids', ids);
    end
