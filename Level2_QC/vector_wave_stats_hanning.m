...
g = 9.81;    % gravity, m/s^2
omega = 2*pi*f_co;   % This is radian frequency

k = get_wavenumber( omega , depth);


correction = zeros(size(f_co));
convert = zeros(size(f_co));

fcutoff = 0.25;

ij = find(f_co <= fcutoff);

correction(ij) = cosh(k(ij)*depth) ./ cosh(k(ij)*doffp);
%limit the correction factor K_p = correction^2 to 10 (order of magnitude)
%this will allow for adjustment in depth and stop overcorrection
%attempted to follow Jones and Monismith 2007 at 12* the noise floor 
%but their sensors are too shallow. 
cutoffFreq = f_co(find(correction.^2 >= 10,1));
correction(correction.^2>10) = sqrt(10);
convert(ij) = (2*pi*f_co(ij)./(g*k(ij))) .* cosh(k(ij)*doffp) ./ cosh(k(ij)*doffu);  % record conversion coefs to check them
%units = m*s^2/(m*s) = s
...
Spec = struct('fm', fmt, 'SSE',SSEt, 'Spp', Sppt,'Suu', Suut, 'Svv', Svvt, 'f_co', f_cot, 'Spu', Sput, 'Spv', Spvt, 'Suv', Suvt, ...
...
dir1 = rad2deg( atan2(b1t,a1t) );
spread1 = rad2deg( sqrt( 2 .* ( 1-sqrt(a1t.^2 + b1t.^2) ) ) );
spread1_ss = nanmean(spread1(i_swell));
spread1_ig = nanmean(spread1(i_ig));

dir2 = rad2deg( atan2(b2t,a2t)/2 );
spread2 = rad2deg(sqrt(abs( 0.5 - 0.5 .* ( a2t.*cos(2.*deg2rad(dir2)) + b2t.*sin(2.*deg2rad(dir2)) )  )));
