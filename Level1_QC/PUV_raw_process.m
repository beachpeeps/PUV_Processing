...
function [PUV] = PUV_raw_process(directory, filename, LATLON, rot_angle, clockdrift)
    ...
    filenames = struct2cell(dir([directory '/' filename '/*.dat'])); filenames = filenames(1,:);
    ...
    for ii = 1:length(filenames)
        id = char(string(ii));
        senfile = sprintf('%s/%s/%s%s.sen',directory, filename, depstr,id)
        datfile = sprintf('%s/%s/%s%s.dat',directory, filename, depstr,id)
    ...
    fid=fopen(sprintf('%s/%s/%s%s.hdr',directory,filename,depstr,'1')); 
    linenum = 12;
