...
p.addOptional('XAngle', '0');
p.addOptional('Nspokes',10);
...
XAngle = p.Results.XAngle ;%Athina
...
if ~ishold(cax);
    ...
    createSpokes(thetaMin,thetaMax,Ncircles,Nspokes, XAngle);
...
% Nested functions
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    function createSpokes(thetaMin,thetaMax,Ncircles,Nspokes, XAngle)
    ...
    function createCircles(rMin,rMax,thetaMin,thetaMax,Ncircles,Nspokes)
        ...
        angleGrid = linspace(thetaMin - XAngle + 90,thetaMax - XAngle + 90,100);
        xGrid = cosd(angleGrid);
        yGrid = sind(angleGrid);
        
        spokeMesh = linspace(thetaMin - XAngle + 90,thetaMax - XAngle + 90,Nspokes);
        ...
        for kk=1:Ncircles
            ...
            position = 1.2.*(spokeMesh(min(Nspokes,round(Ncircles/2)))+...
            ...
            if abs(round(position)) ==90,
                ...
                text((contourD(kk)).*cosd(90-position),...
                    (0.1+contourD(kk)).*sind(90-position),...
                    rtick,'verticalalignment','BaseLine','Color', 'k',...
                    'horizontalAlignment', 'center',...
                    'handlevisibility','off','parent', cax);
                % annotate spokes
                text(contourD(end).*0.6.*cosd(90-position),...
                    0.07+contourD(end).*0.6.*sind(90-position),...
                    [labelR],'verticalalignment','bottom','Color', 'k',...
